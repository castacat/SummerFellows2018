...
#def plot_dist(yd,size):
    ...
    #current_plot = win.addPlot(title="Distance Between Uncorrelated Particles Over Time on "+str(size)+" by "+str(size)+" Grid",labels={'left': "Distance",'bottom':"Time"})
    ##pg.plot(p1x,p1y)#,connect='all',pen='r',symbol='o')
    ##plotWidget.plot(p2x,p2y,connect='all',pen='b',symbol='x')     
    ##y,x = np.histogram(x, y)
    #step=1
